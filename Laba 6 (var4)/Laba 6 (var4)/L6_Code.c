...
const char *MENU[] = { "| 0 | Quit.", "| 1 | Add room.", "| 2 | Add corridor.", "| 3 | Delete room.", "| 4 | Find exit.", "| 5 | Show labirint.", "| 6 | Launch profiler." };
...
const int(*fptr[])(Room *) = { NULL, D_AddR, D_AddC, D_DelR, D_FindE, D_Show, D_Profiler };
...
int deleteR(Room *L, char *name)
{
	...
	while (ral) {
		...
		if ((sr->Adj->pos.x == r->pos.x) && (sr->Adj->pos.y == r->pos.y)) {
			t = sr->Adj;
			sr->Adj = sr->Adj->next;
			free(t);
		}
			...
			sal = sr->Adj;
			while ((sal->next) && !((sal->next->pos.x == r->pos.x) && (sal->next->pos.y == r->pos.y))) sal = sal->next;
			if (!sal->next) return 3;
			t = sal->next;
